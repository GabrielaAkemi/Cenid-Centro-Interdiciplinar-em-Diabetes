...
const inputClasses =
  "mt-1 block w-full rounded-md border-gray-300 text-gray-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
const tableInputClasses =
  "w-full p-1 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 text-sm text-gray-900"
                ...
                <tr key={i} className="border-b border-gray-200 hover:bg-gray-50">
                  <td className="p-1">
                    <input
                      type="text"
                      value={pos.posologia}
                      onChange={(e) => handlePosologiaChange(i, "posologia", e.target.value)}
                  ...
                  <td className="p-1">
                    <input
                      type="text"
                      value={pos.frequencia}
                      onChange={(e) => handlePosologiaChange(i, "frequencia", e.target.value)}
                      className={tableInputClasses}
                    />
                  </td>
                  <td className="p-1">
                    <input
                      type="text"
                      value={pos.dose}
                      onChange={(e) => handlePosologiaChange(i, "dose", e.target.value)}
                      className={tableInputClasses}
                    />
                  </td>
                  <td className="p-1">
                    <input
                      type="text"
                      value={pos.horario}
                      onChange={(e) => handlePosologiaChange(i, "horario", e.target.value)}
                      className={tableInputClasses}
                    />
                  </td>
                  <td className="p-1">
    ...
    <div className="questionnaire-section bg-blue-50 p-6 rounded-2xl border border-blue-200 mt-6">
      ...
      {(
        ...
        <div key={q.key} className="form-field mb-2">
          ...
          <select name={q.key} value={data[q.key as keyof BaasisData]} onChange={handleChange} className={inputClasses}>
            <option value="">Selecione</option>
            {q.options.map((v) => (
  ...
  return (
    <div className="questionnaire-section bg-green-50 p-6 rounded-2xl border border-green-200 mt-6">
      <h4 className="text-xl font-semibold text-green-800 mb-2">
        BAASIS-CSII para usuários de bomba de insulina (SICI)
      </h4>
      <p className="text-sm text-gray-600 mb-4">
...
const InsulinAdherence: React.FC<InsulinAdherenceProps> = ({ onChange }) => {
  ...
  const [questionnaireData, setQuestionnaireData] = useState<BaasisData | BaasisCSIIData>({} as BaasisData)

  const handleMethodChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newMethod = e.target.value
    setMethod(newMethod)
    setQuestionnaireData({} as BaasisData)
      ...
      {medicationCategories
        ...
        .map((cat) => (
          <div
            key={cat.key}
            className={
              cat.key === "vitaminas" ? "bg-white p-6 rounded-2xl border border-gray-200 mb-6 shadow-sm" : cardClasses
            }
          >
            ...
            {meds[cat.key] && !cat.isParent && (
              <MedicationCard medicationData={meds[cat.key]} onDataChange={(data) => handleDataChange(cat.key, data)} />
            )}
            {cat.isParent && (
              <div className="pl-4 mt-4 space-y-4">
                {medicationCategories
                  .filter((subCat) => subCat.parent === cat.key)
                  .map((subCat) => (
    ...
    <div className="bg-gray-100 min-h-screen p-4 sm:p-8 flex items-center justify-center font-sans">
      <div className="bg-white rounded-3xl shadow-2xl p-6 sm:p-10 w-full max-w-6xl">
        ...
        <p className="text-center text-gray-600 mb-10">
          A avaliação da Farmácia deve ser feita sempre que o paciente realizar uma nova consulta médica com mudança na
