...
// ... (Nenhuma alteração nos componentes MedicationCard, BAASIS, BAASIS_CSII, etc.)
...
const App: React.FC<AppProps> = ({ patientData, initialData }) => {
    ...
    // NOVO: Estado para controlar o status da consulta
    ...
    useEffect(() => {
        ...
        if (initialData) {
            ...
            // NOVO: Define o status com base nos dados iniciais.
            // Supondo que `initialData` tenha uma propriedade `status`
            setStatus(initialData.status === 'concluida' ? 'concluida' : 'andamento');
            ...
            // NOVO: Uma nova consulta sempre começa como 'em andamento'
            setStatus('andamento');
            return;
        }
    }, [initialData, patientData]);
    
    // NOVO: Função para alternar o status da consulta
    const handleStatusChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setStatus(e.target.checked ? 'concluida' : 'andamento');
    };

    // NOVO: Função para obter classes CSS com base no status
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const payload: any = {
            ...
            status: status // NOVO: Enviar o status da consulta para a API
        };
        ...
        const objCriado: any = await apiFetch("/api/consulta-farmacia/", true, {
            method: "POST",
    ...
    return (
        <div className="flex flex-col min-h-screen bg-white font-sans p-6 text-gray-800">
            {/* NOVO: Classe do container agora é dinâmica com base no status */}
            <div className={`max-w-4xl mx-auto w-full p-8 rounded-lg shadow-xl border-2 transition-colors duration-300 ${getStatusContainerClasses()}`}>
                ...
                {/* NOVO: Header com título e checkbox de status */}
